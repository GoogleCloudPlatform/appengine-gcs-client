      ...
      try:
        ...
        with cloudstorage.open(final_file, 'r') as gcs_source:
          ...
          while line:
            ...
            counter += 1
            line = gcs_source.readline()
        stats_array.append(str(counter))
        stats_array.append('</p>')
      except errors.NotFoundError:
        stats_array.append('Error opening file</p>')
        output_array.append('Error opening file')
    except (errors.NotFoundError, ValueError, TypeError) as the_error:
      stats_array.append('Compose threw: %s</p>' % str(the_error))
      output_array.append('Compose threw: %s</p>' % str(the_error))
    output_array.append('********************************************************************</p>')

  return ''.join(stats_array), ''.join(output_array)
